...
%       Fission : Probabilité qu'un noyau absorbe un neutron et fissionne
%       Capture : Probabilité qu'un noyau absorbe un neutron et fissionne 
%
...
%
%   ETAPES : etapes intermédiaires pour construire la data base :
%       1°) Construction d'une data base pour les elements Ux et Np9 et Pu9
%       où Ux peuvent soit fissioner soit capturer jusqu'à U9. Puis U9 à
%       Np9 peuvent soit fissionner soit beta -. Pu peut juste fissioner
%
%   SOURCES : fichier viennent de
...
counter=1;
sigma=0*n_eV;%Allcoation taille
for ii=1:length(n_eV)% Il existe des absysses à plusiuuers ordonnées :/
    for jj=counter:(length(E))
        if(((n_eV(ii)-E(jj))/n_eV(ii))<10^-6)% Différence relative négligeable. Ceci a cause d'erreur d'arrondis :/
            sigma(ii)=Sig(jj);
            counter=jj;
            break;
        end
